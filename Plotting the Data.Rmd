...
```{r}
ECMPlotlong <- pivot_longer(ECMPlot, cols = starts_with("Mean"), names_to = "Channel", values_to = "Mean_Expression")
...
```{r}
ECMPlotlong %>% filter(Metadata_Treatment == "DMSO") %>% ggplot(aes(x = Mean_Expression, y = Condition, fill = Channel))+
