...
VEPECMmeansNuc %>% group_by(Metadata_Stiffness, CollagenIV) %>% summarize(Mean_PXR = mean(Mean_PXR)) %>% ggplot( aes(x = CollagenIV, y = Mean_PXR, col = Metadata_Stiffness))+
  ...
  ggsave("PXR Intx/StiffnessvsC4PXR.png")
 ...
 #  ggsave("PXR Intx/StiffnessvsOPNPXR.png")
  ...
  ylab("PXR Expression")+
  scale_color_manual(values = c("#db6d22", "#2100bb"))+
    ggtitle("PXR - Decorin Ratio Over Stiffness")
...
VEPECMmeansNuc %>% group_by(Metadata_Stiffness, Decorin) %>% summarize(Mean_Ecad = mean(Mean_Ecad)) %>%ggplot( aes(x = Decorin, y = Mean_Ecad, col = Metadata_Stiffness))+
  ...
  xlab("Ratio of Decorin")+
  ylab("Ecad Expression")+
  ggtitle("Ecad - Decorin Ratio over Stiffness")+
  scale_color_manual(values = c("#db6d22", "#2100bb"))

  ggsave("Ecad Intx/StiffnessvsDNEcad.png")
  
  VEPECMmeansNuc %>% group_by(Metadata_Stiffness, Laminin) %>% summarize(Mean_Ecad = mean(Mean_Ecad)) %>%ggplot( aes(x = Laminin, y = Mean_Ecad, col = Metadata_Stiffness))+
  #facet_grid(cols = vars(Metadata_Stiffness))+
  geom_path(aes(group = Metadata_Stiffness), linewidth = 1.5)+
  geom_point(size = 3)+
  theme_prism(border = TRUE)+
  ...
  ggsave("Ecad Intx/StiffnessvsLNEcad.png")
  
  VEPECMmeansNuc %>% group_by(Metadata_Stiffness, Fibronectin) %>% summarize(Mean_Ecad = mean(Mean_Ecad)) %>%ggplot( aes(x = Fibronectin, y = Mean_Ecad, col = Metadata_Stiffness))+
  #facet_grid(cols = vars(Metadata_Stiffness))+
  geom_path(aes(group = Metadata_Stiffness), linewidth = 1.5)+
  geom_point(size = 3)+
  theme_prism(border = TRUE)+
  ...
  ggsave("Ecad Intx/StiffnessvsFNEcad.png")
```

### VIM Sig R:ECM

```{r}
VEPECMmeansNuc %>% filter(TenascinC != 0) %>% group_by(Abb_ECM, TenascinC) %>% summarize(Mean_Vim = mean(Mean_Vim)) %>%ggplot( aes(x = Abb_ECM, y = Mean_Vim, col = TenascinC))+
  #facet_grid(cols = vars(Metadata_Stiffness))+
  geom_path(aes(group = TenascinC, linetype = TenascinC), linewidth = 1.5)+
  geom_point(size = 3)+
  theme_prism(border = TRUE)+
  xlab("ECM Composition")+
  labs(color = "Tenascin C")+
  ...
  ggtitle("Tenascin C Ratio over ECMs")
  
ggsave("Vim Intx/TCRatiovsECM.png", width = 10, height = 5)
...
VEPECMmeansNuc %>% filter(Hyaluronic_Acid != 0) %>% group_by(Abb_ECM, Hyaluronic_Acid) %>% summarize(Mean_Vim = mean(Mean_Vim)) %>%ggplot( aes(x = Abb_ECM, y = Mean_Vim, col = Hyaluronic_Acid))+
  ...
  scale_linetype_manual(values = c("longdash", "solid", "twodash", "solid"))+
  ylab("Vimentin Expression")+
  ggtitle("Vimentin - Hyaluronic Acid Ratio over ECMs")
  
ggsave("Vim Intx/Hyaluronic_AcidRatiovsECM.png", width = 10, height = 5)
...
ggsave("Vim Intx/FibronectinRatiovsECM.png", width = 10, height = 5)
...
ggsave("Vim Intx/Galectin3RatiovsECM.png", width = 10, height = 5)
```
### Ecad Sig R:ECM
  C1, HA
```{r}
VEPECMmeansNuc %>% filter(CollagenI != 0) %>% group_by(Abb_ECM, CollagenI) %>% summarize(Mean_Ecad = mean(Mean_Ecad)) %>%ggplot( aes(x = Abb_ECM, y = Mean_Ecad, col = CollagenI))+
  #facet_grid(cols = vars(Metadata_Stiffness))+
  ...
  scale_linetype_manual(values = c("longdash", "solid", "twodash", "solid"))+
  ylab("Ecad Expression")+
  ggtitle("Ecad - Collagen I Ratio over ECMs")
  
ggsave("Ecad Intx/CollagenIRatiovsECMEcad.png", width = 10, height = 5)

VEPECMmeansNuc %>% filter(Hyaluronic_Acid != 0) %>% group_by(Abb_ECM, Hyaluronic_Acid) %>% summarize(Mean_Ecad = mean(Mean_Ecad)) %>%ggplot( aes(x = Abb_ECM, y = Mean_Ecad, col = Hyaluronic_Acid))+
  #facet_grid(cols = vars(Metadata_Stiffness))+
  geom_path(aes(group = Hyaluronic_Acid, linetype = Hyaluronic_Acid), linewidth = 1.5)+
  geom_point(size = 3)+
  theme_prism(border = TRUE)+
  ...
  scale_linetype_manual(values = c("longdash", "solid", "twodash", "solid"))+
  ylab("Ecad Expression")+
  ggtitle("Ecad - Hyaluronic Acid Ratio over ECMs")
  
ggsave("Ecad Intx/HARatiovsECMEcad.png", width = 10, height = 5)
```
### Vimentin 3-way Intx
G3:ECM:Stiffness
```{r}
VEPECMmeansNuc %>% filter(Galectin3 != 0) %>% group_by(Abb_ECM, Galectin3, Metadata_Stiffness) %>% summarize(Mean_Vim = mean(Mean_Vim)) %>%ggplot( aes(x = Abb_ECM, y = Mean_Vim, col = Galectin3))+
  facet_grid(cols = vars(Metadata_Stiffness))+
  geom_path(aes(group = Galectin3, linetype = Galectin3), linewidth = 1.5)+
  geom_point(size = 3)+
  theme_prism(border = TRUE)+
  ...
  scale_linetype_manual(values = c("longdash", "solid", "twodash", "solid"))+
  ylab("Vimentin Expression")+
  ggtitle("Vimentin - Galectin 3 Ratio over ECMs and Stiffnesses")
  
ggsave("Vim Intx/Galectin3RatiovsECMvsStiffness.png", width = 15, height = 5)
```
...
Need to Make Expanded Versions Prior to Running
#### P-value Heatmap
```{r}
...
```{r}
## Mean_Green
DMSOdsewithrep <- DMSOdsewithrep %>% mutate(Conditions = paste0(Abb_ECM, Metadata_Ratio))
...
```{r}
library(tidyverse)
controlmean_1218 <- ds_pheno %>% filter(Metadata_Treatment == "DMSO") %>% group_by(Abb_ECM, Metadata_Ratio, Metadata_Stiffness, Metadata_Experiment)
...
FOC12_18 <- left_join(ds_pheno, controlmeanfinal)
...
SP1 <- SP1[,c("FOC", "Mean_Orange_Norm", "Mean_Red_Norm", "Mean_Green_Norm")]
SP25 <- SP25[,c("FOC", "Mean_Orange_Norm", "Mean_Red_Norm", "Mean_Green_Norm")]
...
corrplot(SPcor1, p.mat = testRes1$p, sig.level = 0.05, type = "upper",  col = brewer.pal(n=8, name = "RdYlBu"), diag = FALSE, addCoef.col = 'black', tl.col = "black", tl.srt = 0)
mtext("Correlations on 1 kPa", side = 3, line = 2.5, cex = 2)
dev.off()

png("SPcor25corr.png", width = 6, height = 6, units = "in", res = 1200)
corrplot(SPcor25, p.mat = testRes25$p, sig.level = 0.05, type = "upper", col = brewer.pal(n=8, name = "RdYlBu"), diag = FALSE, addCoef.col = 'black',tl.col = "black", tl.srt = 0)
mtext("Correlation on 25 kPa", side = 3, line = 2.5, cex = 2)
dev.off()
...
```{r}
cor.test(ds_pheno$Mean_Green_Norm, ds_pheno$Mean_Red_Norm)
cor.test(ds_pheno$Mean_Green_Norm, ds_pheno$Mean_Orange_Norm)
cor.test(ds_pheno$Mean_Orange_Norm, ds_pheno$Mean_Red_Norm)
```


## Specific ECM Callouts Graph
```{r}
ECMPlot <- ds_pheno %>% mutate(RatioECM1 = case_when(Metadata_Ratio == "100" ~ "100",
...
```{r}
ECMPlot <- ECMPlot[,c("Metadata_Stiffness", "Metadata_Treatment", "Condition", "Mean_Green_Norm", "Mean_Orange_Norm", "Mean_Red_Norm")]

ECMPlot <- subset(ECMPlot, Metadata_Treatment == "DMSO")
...
```{r}
ECMsforPlot <- c("75LN_25DN", "50C1_50HA", "75LN_25HA", "75LN_25G3", "50TC_50HA")

ECMsPlot <- subset(ECMsPlot, Condition %in% ECMsforPlot)
```
...
```{r}
library(dplyr)
dswithrep$Metadata_Treatment <- factor(dswithrep$Metadata_Treatment, levels = c("DMSO", "10 uM"))

dswithrep %>% dplyr::select("Metadata_Stiffness", "Metadata_Treatment", "Conditions", "Mean_Green_Norm", "Mean_Orange_Norm", "Mean_Red_Norm") %>% pivot_longer(cols = starts_with("Mean_"), names_to = "Channel", values_to = "Mean_Expression") %>% 
...
ggsave("Respanovacomp.png", width = 12, height = 10, dpi = 1200)
```
...
DrugResponsesforComplong$Terms <- factor(DrugResponsesforComplong$Terms, levels = unique(DrugResponsesforComplong$Terms))
...
```{r}
dsewithrep %>% group_by(Conditions, Metadata_Stiffness)%>% 
...
dswithrep %>% dplyr::select("Metadata_Stiffness", "Metadata_Treatment", "Conditions", "Mean_Green_Norm", "Mean_Orange_Norm", "Mean_Red_Norm") %>% pivot_longer(cols = starts_with("Mean_"), names_to = "Channel", values_to = "Mean_Expression") %>% 
  mutate(Phenotype = case_when(Channel == "Mean_Green_Norm" ~ "PXR", Channel == "Mean_Red_Norm" ~ "Ecad", Channel == "Mean_Orange_Norm" ~"Vimentin"))%>% filter(Metadata_Treatment == "DMSO")%>%
  ggplot(aes(x = Metadata_Stiffness, y = Mean_Expression ))+
  facet_grid(cols = vars(Phenotype))+
  theme_prism()+
  geom_boxplot()+
  ggtitle("Mean Expression on Stiffnesses")+
  xlab("Stiffness (kPa)")+
  ylab("Mean Expression (aiu)")+
  stat_compare_means(label = "p.signif", size = 6)+
  stat_summary(fun=mean, geom="point", shape=8, size=3, color="red", fill="red") 
  
ggsave("DMSOStiffnessbox.png", width = 5, height = 5)

```
## Ratio Paths
```{r}
RatioCol1 <- dswithrep %>% group_by(CollagenI)%>% summarize(ECAD = mean(Mean_Red_Norm), Vimentin = mean(Mean_Orange_Norm), PXR = mean(Mean_Green_Norm), ECMRatio = "Collagen I")%>% rename("Ratio" = "CollagenI")
RatioCol4<- dswithrep %>% group_by(CollagenIV)%>% summarize(ECAD = mean(Mean_Red_Norm), Vimentin = mean(Mean_Orange_Norm), PXR = mean(Mean_Green_Norm), ECMRatio = "Collagen IV")%>% rename("Ratio" = "CollagenIV")
RatioHA<- dswithrep %>% group_by(Hyaluronic_Acid)%>% summarize(ECAD = mean(Mean_Red_Norm), Vimentin = mean(Mean_Orange_Norm), PXR = mean(Mean_Green_Norm), ECMRatio = "Hyaluronic Acid")%>% rename("Ratio" = "Hyaluronic_Acid")
RatioTC<- dswithrep %>% group_by(TenascinC)%>% summarize(ECAD = mean(Mean_Red_Norm), Vimentin = mean(Mean_Orange_Norm), PXR = mean(Mean_Green_Norm), ECMRatio = "Tenascin C")%>% rename("Ratio" = "TenascinC")
RatioOPN<- dswithrep %>% group_by(Osteopontin)%>% summarize(ECAD = mean(Mean_Red_Norm), Vimentin = mean(Mean_Orange_Norm), PXR = mean(Mean_Green_Norm), ECMRatio = "Osteopontin")%>% rename("Ratio" = "Osteopontin")
RatioG3<- dswithrep %>% group_by(Galectin3)%>% summarize(ECAD = mean(Mean_Red_Norm), Vimentin = mean(Mean_Orange_Norm), PXR = mean(Mean_Green_Norm), ECMRatio = "Galectin 3")%>% rename("Ratio" = "Galectin3")
RatioFN<- dswithrep %>% group_by(Fibronectin)%>% summarize(ECAD = mean(Mean_Red_Norm), Vimentin = mean(Mean_Orange_Norm), PXR = mean(Mean_Green_Norm), ECMRatio = "Fibronectin")%>% rename("Ratio" = "Fibronectin")
RatioLN<- dswithrep %>% group_by(Laminin)%>% summarize(ECAD = mean(Mean_Red_Norm), Vimentin = mean(Mean_Orange_Norm), PXR = mean(Mean_Green_Norm), ECMRatio = "Laminin")%>% rename("Ratio" = "Laminin")

RatioDN <- dswithrep %>% group_by(Decorin)%>% summarize(ECAD = mean(Mean_Red_Norm), Vimentin = mean(Mean_Orange_Norm), PXR = mean(Mean_Green_Norm), ECMRatio = "Decorin")%>% rename("Ratio" = "Decorin")

RatioForInt <- rbind(RatioCol1, RatioCol4, RatioHA, RatioTC, RatioOPN, RatioG3, RatioFN, RatioLN, RatioDN)
```

```{r}
RatioForInt %>% filter(ECMRatio %in% c("Osteopontin", "Hyaluronic Acid", "Laminin", "CollagenI"))%>%ggplot( aes(x = Ratio, y = ECAD, col = ECMRatio))+
...
RatioForInt %>% filter(ECMRatio %in% c("Collagen IV", "Hyaluronic Acid","Tenascin C", "Laminin", "Fibronectin", "Osteopontin"))%>%ggplot( aes(x = Ratio, y = Vimentin, col = ECMRatio))+
  geom_path(aes(group = ECMRatio), linewidth = 1.5)+
  geom_point(size = 3)+
  theme_prism(border = TRUE)+
  xlab("Ratio of ECM")+
...
RatioForInt %>% filter(ECMRatio %in% c("Collagen I","Collagen IV", "Hyaluronic Acid", "Laminin"))%>%ggplot( aes(x = Ratio, y = PXR, col = ECMRatio))+
  geom_path(aes(group = ECMRatio), linewidth = 1.5)+
  geom_point(size = 3)+
  theme_prism(border = TRUE)+
  xlab("Ratio of ECM")+
...
## Dot Plots
```{r}
cor.test(SP1$Vimentin, SP1$PXR, use = "complete.obs")
cor.test(SP1$Ecad, SP1$PXR, use = "complete.obs")
cor.test(SP1$Vimentin, SP1$Ecad, use = "complete.obs")

cor.test(SP25$Vimentin, SP25$PXR, use = "complete.obs")
cor.test(SP25$Ecad, SP25$PXR, use = "complete.obs")
cor.test(SP25$Vimentin, SP25$Ecad, use = "complete.obs")
```

## Updated Correlation Calculation to Match stat_cor facetting 3-19-25
```{r}
dswithrepcor <- dswithrep %>% filter(Metadata_Treatment == "DMSO") %>%group_by(Metadata_Stiffness, Conditions) %>% dplyr::summarize(Mean_PXR = mean(Mean_Green_Norm), Mean_Vimentin = mean(Mean_Orange_Norm), Mean_Ecad = mean(Mean_Red_Norm))

cor.test(subset(dswithrepcor, Metadata_Stiffness == "1kPa")$Mean_Vimentin, subset(dswithrepcor, Metadata_Stiffness == "1kPa")$Mean_PXR)
cor.test(subset(dswithrepcor, Metadata_Stiffness == "1kPa")$Mean_Ecad, subset(dswithrepcor, Metadata_Stiffness == "1kPa")$Mean_PXR)
cor.test(subset(dswithrepcor, Metadata_Stiffness == "1kPa")$Mean_Vimentin, subset(dswithrepcor, Metadata_Stiffness == "1kPa")$Mean_Ecad)

cor.test(subset(dswithrepcor, Metadata_Stiffness == "25kPa")$Mean_Vimentin, subset(dswithrepcor, Metadata_Stiffness == "25kPa")$Mean_PXR)
cor.test(subset(dswithrepcor, Metadata_Stiffness == "25kPa")$Mean_Ecad, subset(dswithrepcor, Metadata_Stiffness == "25kPa")$Mean_PXR)
cor.test(subset(dswithrepcor, Metadata_Stiffness == "25kPa")$Mean_Vimentin, subset(dswithrepcor, Metadata_Stiffness == "25kPa")$Mean_Ecad)
```
...
SPforcor <- SurvivalPheno %>% filter(Metadata_Treatment == "DMSO") %>% group_by(Metadata_Stiffness, Condition)%>% summarize(Survival = mean(FOC, na.rm = TRUE), Ecad = mean(Mean_Red_Norm, na.rm = TRUE), Vimentin = mean(Mean_Orange_Norm, na.rm = TRUE), PXR = mean(Mean_Green_Norm, na.rm = TRUE))

SPforcor1 <- subset(SPforcor, Metadata_Stiffness == "1kPa")
SPforcor25 <- subset(SPforcor, Metadata_Stiffness == "25kPa")

SPforcor1 <- SPforcor1[, -c(1,2)]
SPforcor25 <- SPforcor25[, -c(1,2)]

NewSPcor1 <- cor(SPforcor1)
NewSPcor25 <- cor(SPforcor25)

NewtestRes1<- cor.mtest(SPforcor1)
NewtestRes25 <- cor.mtest(SPforcor25)

png("NewSPcor1corr.png", width = 6, height = 6, units = "in", res = 1200)
corrplot(NewSPcor1, p.mat = NewtestRes1$p, sig.level = 0.05, type = "upper",  col = brewer.pal(n=8, name = "RdYlBu"), diag = FALSE, addCoef.col = 'black', tl.col = "black", tl.srt = 0)
mtext("Correlations on 1 kPa", side = 3, line = 2.5, cex = 2)
dev.off()

png("NewSPcor25corr.png", width = 6, height = 6, units = "in", res = 1200)
corrplot(NewSPcor25, p.mat = NewtestRes25$p, sig.level = 0.05, type = "upper", col = brewer.pal(n=8, name = "RdYlBu"), diag = FALSE, addCoef.col = 'black',tl.col = "black", tl.srt = 0)
mtext("Correlation on 25 kPa", side = 3, line = 2.5, cex = 2)
dev.off()
```
...
dswithrep %>% filter(Metadata_Treatment == "DMSO") %>%group_by(Metadata_Stiffness, Conditions) %>% dplyr::summarize(Mean_PXR = mean(Mean_Green_Norm), Mean_Vimentin = mean(Mean_Orange_Norm)) %>% ggplot(aes(Mean_PXR, Mean_Vimentin)) + 
  facet_grid(cols = vars(Metadata_Stiffness))+
...
ggsave("VimPXRScatter.png" , dpi = 1200)

dswithrep %>% filter(Metadata_Treatment == "DMSO") %>%group_by(Metadata_Stiffness, Conditions) %>% dplyr::summarize(Mean_PXR = mean(Mean_Green_Norm), Mean_Ecad = mean(Mean_Red_Norm)) %>% ggplot(aes(Mean_PXR, Mean_Ecad)) + 
  facet_grid(cols = vars(Metadata_Stiffness))+
  geom_point(aes(color = Conditions))+
  theme_prism(border = TRUE) +
  #scale_x_log10()+
  ...
  xlab("PXR Expression")+
  stat_cor(method = "pearson")+
  ylab("E-cadherin Expression")
ggsave("EcadPXRScatter.png" , dpi = 1200)

dswithrep %>% filter(Metadata_Treatment == "DMSO") %>%group_by(Metadata_Stiffness, Conditions) %>% dplyr::summarize(Mean_Ecad = mean(Mean_Red_Norm), Mean_Vimentin = mean(Mean_Orange_Norm)) %>% ggplot(aes(Mean_Ecad, Mean_Vimentin)) + 
  facet_grid(cols = vars(Metadata_Stiffness))+
  geom_point(aes(color = Conditions))+
  theme_prism(border = TRUE) +
  #scale_x_log10()+
  theme(legend.position = "none")+
  xlab("E-cadherin Expression")+
  stat_cor(method = "pearson")+
  ylab("Vimentin Expression")
ggsave("VimEcadScatter.png" , dpi = 1200)
```
